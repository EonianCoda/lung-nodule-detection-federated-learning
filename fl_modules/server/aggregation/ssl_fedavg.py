...
class FedAvg(Aggregation):
    ...
    def __call__(self, clients: Dict[str, Client], client_weights: Dict[str, float], round_number: int) -> Tuple[Dict[str, Any], Dict[str, Any], Dict[str, Any], Dict[str, Any]]:
        ...
        aggregated_model_s_state_dict = {key: torch.zeros_like(value) for key, value in self.model.state_dict().items()}
        customized_model_s_state_dict = dict()
        
        aggregated_model_t_state_dict = {key: torch.zeros_like(value) for key, value in self.model.state_dict().items()}
        customized_model_t_state_dict = dict()
        
        aggregated_optimizer_state_dict = dict()
        customized_optimizer_state_dict = dict()
        
        for client_name, client in clients.items():
            # Aggregate student model
            client.load_model_state(self.model, round_number, self.cpu_device, load_teacher = False)
            ...
            for key, value in local_model_state_dict.items():
                ...
                if key in self.model_keep_local_state:
                    # Lazy initialization
                    if client_name not in customized_model_s_state_dict:
                        customized_model_s_state_dict[client_name] = dict()
                    customized_model_s_state_dict[client_name][key] = value
                # aggregated model state
                elif value is not None:
                    if value.dtype == torch.long:
                        aggregated_model_s_state_dict[key] += torch.round(client_weights[client_name] * value).long()
                    else:
                        aggregated_model_s_state_dict[key] += client_weights[client_name] * value
                
            # Aggregate teacher model
            client.load_model_state(self.model, round_number, self.cpu_device, load_teacher = True)
            local_model_state_dict = self.model.state_dict()
            for key, value in local_model_state_dict.items():
                # customized model state
                if key in self.model_keep_local_state:
                    # Lazy initialization
                    if client_name not in customized_model_t_state_dict:
                        customized_model_t_state_dict[client_name] = dict()
                    customized_model_t_state_dict[client_name][key] = value
                # aggregated model state
                elif value is not None:
                    if value.dtype == torch.long:
                        aggregated_model_t_state_dict[key] += torch.round(client_weights[client_name] * value).long()
                    else:
                        aggregated_model_t_state_dict[key] += client_weights[client_name] * value
        ...
        return aggregated_model_s_state_dict, customized_model_s_state_dict, aggregated_model_t_state_dict, customized_model_t_state_dict, aggregated_optimizer_state_dict, customized_optimizer_state_dict