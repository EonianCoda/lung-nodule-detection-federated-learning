...
def compute_sphere_volume(h, w, d):
    """Compute the volume of a sphere given its diameter.
    Args:
        diameter: float
        ...
        if not os.path.exists(cache_path):
            ...
            if last_modified_time == nodule_count[LAST_MODIFIED_TIME]:
                ...
                if len(nodule_count[BBOXES]) != 0:
                    gt_bbox = nodule_count[BBOXES]
                    gt_bbox = np.array(gt_bbox)
                    hwd = gt_bbox[:, 1, :] - gt_bbox[:, 0, :]
                    volumes = [compute_sphere_volume(*d) for d in hwd]
                    volumes = np.array(volumes)
                    remove_mask = volumes < 5
                    volumes = np.array(volumes)[~remove_mask]
                    gt_valid_nodule_sizes = np.array(volumes).tolist()
            else:
                gt_valid_nodule_sizes = NoduleCounter.get_and_write_num_of_nodules(gt_mask_map_path, cache_path)
